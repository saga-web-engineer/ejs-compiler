...
- Watch mode (`-w, --watch`): keep the process running and recompile only the changed file on save

### Fixed

- Normalize plain-name exclude patterns (e.g. `_*`) to `**/_*` so they exclude matching files at any directory depth, not just the root
- Correct Node.js requirement from 18+ to 24+ in README
